...
fn main() -> eframe::Result { env_logger::init();   
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default().with_resizable(false).with_inner_size([320.0, 240.0]),
        ..Default::default()
    };
    eframe::run_native(
        "Playerok Activator",
...
struct MyApp {
    result: String,
    activate_first: bool,
    activate_second: bool
}

impl Default for MyApp {
    fn default() -> Self {
        Self {
            result: String::from("Ожидание действия"),
...
impl eframe::App for MyApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.heading("Активация Windows");
            if ui.button("Windows 10/11").clicked() {
               self.result = String::from("Начата активация Windows 10/11...\nПожалуйста, подождите.");
               self.activate_first = true;
            }
            if ui.button("Windows LTSC").clicked() { 
               self.result = String::from("Начата активация Windows LTSC...\nПожалуйста, подождите.");
               self.activate_second = true;
            }
            ui.label(format!("{}", &self.result));

            if self.activate_first {
                first_activate();
                self.result = String::from("Активация Windows 10/11 завершена.\nОстались вопросы, активация не прошла?\nОбратитесь в личные сообщения, помогу вам в любое время.");
                //activate(FIRST_ACTIVATION_CODE);
                self.activate_first = false;
            }
            if self.activate_second {
                self.result = String::from("Активация Windows LTSC завершена.\nОстались вопросы, активация не прошла?\nОбратитесь в личные сообщения, помогу вам в любое время.");
                //activate(SECOND_ACTIVATION_CODE);
                self.activate_second = false;
            }
        });
    }
}
